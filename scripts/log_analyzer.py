...
def analyze():
    ...
    valid_alts = [h for h in alts if abs(h) < 500]
    max_valid_alt = max(valid_alts) if valid_alts else 0
    ekf_drifted = len(valid_alts) < len(alts) * 0.5
    ...
    if max_roll > 60.0 or max_pitch > 60.0:
        print(f"RESULT: CRASH | max_roll={max_roll:.1f}° max_pitch={max_pitch:.1f}°"); return
    if ekf_drifted:
        print(f"RESULT: UNSTABLE | EKF drifted — valid={len(valid_alts)}/{len(alts)}"); return
    if osc_rate > 3.0 or max_roll > 25.0 or max_pitch > 25.0:
        print(f"RESULT: OSCILLATING | osc_rate={osc_rate:.1f}/s max_roll={max_roll:.1f}°"); return
    if max_roll < 8.0 and max_pitch < 8.0 and max_valid_alt > 0.3 and avg_thr > 0.3:
        print(f"RESULT: STABLE | max_roll={max_roll:.1f}° max_pitch={max_pitch:.1f}° max_alt={max_valid_alt:.2f}m"); return
    print(f"RESULT: UNSTABLE | max_roll={max_roll:.1f}° max_pitch={max_pitch:.1f}° max_alt={max_valid_alt:.2f}m")
