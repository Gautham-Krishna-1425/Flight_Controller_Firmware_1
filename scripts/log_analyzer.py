...
def analyze():
    ...
    # Detect oscillation using roll sign changes
    crossings = sum(1 for i in range(1, len(rolls)) if rolls[i]*rolls[i-1] < 0)
    osc_rate  = crossings / (data[-1]["t"] + 0.001)

    # Altitude check — ignore EKF drift by checking relative change
    alts = [d["h_est"] for d in data]
    alt_range = max(alts) - min(alts)
    lifted = alt_range > 0.3  # moved more than 30cm vertically

    if max_roll > 60.0 or max_pitch > 60.0:
        print(f"RESULT: CRASH | max_roll={max_roll:.1f}° max_pitch={max_pitch:.1f}°")
    elif osc_rate > 3.0 or (max_roll > 25.0 and max_pitch > 25.0):
        print(f"RESULT: OSCILLATING | osc_rate={osc_rate:.1f}/s max_roll={max_roll:.1f}°")
    elif max_roll < 8.0 and max_pitch < 8.0 and avg_thr > 0.4:
        print(f"RESULT: STABLE | max_roll={max_roll:.1f}° max_pitch={max_pitch:.1f}° avg_thr={avg_thr:.2f}")
    else:
        print(f"RESULT: UNSTABLE | max_roll={max_roll:.1f}° max_pitch={max_pitch:.1f}° osc={osc_rate:.1f}/s")
